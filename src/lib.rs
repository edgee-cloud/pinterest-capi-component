...
impl Guest for PinterestComponent {
    fn page(edgee_event: Event, settings: Dict) -> Result<EdgeeRequest, String> {
        if let Data::Page(ref data) = edgee_event.data {
            ...
            Ok(build_edgee_request(pinterest_payload))
        } else {
    ...
    fn track(edgee_event: Event, settings: Dict) -> Result<EdgeeRequest, String> {
        if let Data::Track(ref data) = edgee_event.data {
            ...
            custom_data.insert("custom_event_name".to_string(), parse_value(&data.name));
            event.custom_data = Some(custom_data);
            pinterest_payload.data.push(event);
...
fn build_edgee_request(pinterest_payload: PinterestPayload) -> EdgeeRequest {
    let headers = vec![
        (
        ...
        (
            ...
            format!("Bearer {}", pinterest_payload.access_token),
        ),
    ];

    let url = format!(
        "https://api.pinterest.com/v5/ad_accounts/{}/events",
        pinterest_payload.ad_account_id
    );
...
mod tests {
    ...
    fn page_with_consent() {
        ...
        assert_eq!(
            edgee_request
                .url
                .starts_with("https://api.pinterest.com/v5/ad_accounts"),
            true
        );
    ...
    fn user_even_with_test_event_code() {
        ...
        assert_eq!(result.clone().is_err(), false);
        let edgee_request = result.unwrap();
        assert_eq!(edgee_request.url.contains("test=true"), true);
    }
    ...
    fn user_event_without_email_or_phone_fails() {
        ...
        assert_eq!(
            result
                ...
                .unwrap()
                .to_string()
                .contains("User properties must contain email"),
            true
        );
    }
